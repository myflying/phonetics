...
Page({
  ...
  onLoad: function(options) {
    ...
    var that = this
    let url = "https://yb.bshu.com/api/v1.index/shortClause"
    var rdata = {
    ...
    wx.request({
      ...
      success: function(result) {
        ...
        for(var t = 0;t<temp.length;t++){
          let itemphrase = ''
          var itemArray = temp[t].phrase.split(' ');
          for (var i = 0; i < itemArray.length; i++) {
            var itemStr = itemArray[i]
            itemphrase += app.changeWords(itemStr)
          }

          temp[t]['new_itemphrase'] = itemphrase
        }
        console.log(temp)
        ...
        //vowel_name = result.data.data[0].phrase.replace(new RegExp('#', 'g'), '').trim();
  ...
  phraseChecked: function(e) {
    ...
    vowel_name = item.phrase.replace(new RegExp('#', 'g'), '').trim();
  ...
  nextStep: function(e) {
    wx.navigateTo({
      url: '/pages/vowel_seven/vowel_seven',
    })
  },
